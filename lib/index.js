// src/workspace.ts
import { existsSync as existsSync2, mkdirSync as mkdirSync2, readFileSync, writeFileSync, rmSync, chmodSync, realpathSync } from "node:fs";
...
// src/workspace.ts
function getBaseDir() {
  const dir = join2(homedir2(), ".dsh", "dsh-ssh");
  if (!existsSync2(dir)) {
...
function getWorkspacesDir() {
  const dir = join2(getBaseDir(), "workspaces");
  if (!existsSync2(dir)) {
    mkdirSync2(dir, { recursive: true });
  }
  return dir;
...
function findRemoteWorkspaceMeta(startPath) {
  ...
  while (current && current !== root) {
    const metaPath = join2(current, ".remote-ssh.json");
...
async function createRemoteWorkspace(workspaceRegistry, host, remotePath, customTitle, authType, password) {
  try {
    ...
    const anchorDir = join2(getWorkspacesDir(), id);
    ...
    writeFileSync(join2(anchorDir, ".remote-ssh.json"), JSON.stringify(meta, null, 2), "utf8");
    const agentsMd = [
    ...
    writeFileSync(join2(anchorDir, "AGENTS.md"), agentsMd, "utf8");
    let workspaceId;
...
`;
  writeFileSync(join2(baseDir, "dsh-remote-shell.js"), runnerJs, "utf8");
  const bashLauncher = `#!/bin/sh
exec node "$(dirname "$0")/dsh-remote-shell.js" "$@"
`;
  const posixPath = join2(baseDir, "dsh-remote-shell");
...
`;
  writeFileSync(join2(baseDir, "dsh-remote-shell.cmd"), cmdLauncher, "utf8");
}
...
function parseSshConfig(customPath) {
  const configPath = customPath || join4(homedir4(), ".ssh", "config");
...
// src/api.ts
import { readdirSync, existsSync as existsSync4, readFileSync as readFileSync3 } from "node:fs";
        ...
        try {
          ...
          if (method === "workspaces") {
            ...
            if (existsSync4(wsDir)) {
              for (const name2 of readdirSync(wsDir)) {
