...
async function remoteReadFile(host, remotePath) {
  ...
  const script = `( [ -f ${shellQuote(remotePath)} ] || { echo '__DSH_ERR_NOT_FOUND__'; exit 1; }; SIZE=$(wc -c < ${shellQuote(remotePath)} 2>/dev/null || stat -c %s ${shellQuote(remotePath)} 2>/dev/null || echo 0); if [ "$SIZE" -gt ${MAX_BYTES} ]; then echo "__DSH_ERR_TOO_LARGE__:$SIZE"; exit 1; fi; base64 < ${shellQuote(remotePath)} 2>/dev/null )`;
  const r = await runSsh(host, script);
  if (!r.ok) {
...
async function remoteWriteFile(host, remotePath, content) {
  ...
  const script = `mkdir -p ${shellQuote(dirname2)} && base64 -d > ${shellQuote(tmpPath)} && mv -f ${shellQuote(tmpPath)} ${shellQuote(remotePath)} || { rm -f ${shellQuote(tmpPath)} 2>/dev/null; exit 1; }`;
...
function hookWorkspaceRegistryDeletion(ctx) {
  ...
  const sep2 = (process.platform === "win32" ? "\\" : "/").toLowerCase();
  const prefixWithSep = wsBaseDir.endsWith(sep2) ? wsBaseDir : wsBaseDir + sep2;
  reg.delete = async function(id) {
    ...
    try {
      ...
      if (entity && entity.path) {
        ...
        try {
          canon = realpathSync(resolvedPath).toLowerCase();
        } catch {
        }
...
function checkRateLimit(host) {
  ...
  if (record.lockedUntil > Date.now()) {
    const remaining = Math.ceil((record.lockedUntil - Date.now()) / 1e3);
